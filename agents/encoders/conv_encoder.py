...
class Conv_pc_rgb_(nn.Module):
    def __init__(self, fuse_type='add'):
        super(Conv_pc_rgb_, self).__init__()
...
class Conv_pc_rgb(nn.Module):
    def __init__(self, fuse_type='add'):
        ...
        self.static_linear = nn.Linear(2048, 768)
        self.gripper_linear = nn.Linear(2048, 768)
    ...
    def forward(self, x):
        ...
        static_tokens = self.static_linear(static_tokens)
        gripper_tokens = self.gripper_linear(gripper_tokens)
...
class Convnext_rgb(nn.Module):
    def __init__(self,):
        super(Convnext_rgb, self).__init__()

        # self.pc_static = ConvNextv2(pretrained=False)
        # self.pc_gripper = ConvNextv2(pretrained=False)

        self.rgb_static = ConvNextv2(pretrained=True)
        self.rgb_gripper = ConvNextv2(pretrained=True)
