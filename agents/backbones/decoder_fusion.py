...
class Dec_only(nn.Module):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, Dec_only):
...
class Noise_Dec_only(nn.Module):
    def __init__(
        ...
        # linear embedding for the state
        self.rgb_emb = nn.Linear(rgb_dim, embed_dim)
    ...
    def forward(
        ...
        rgb_states = states['rgb_tokens']
        pc_states = states['pc_tokens']

        # t for the states does not mean the time, but the number of inputs tokens
        b, t, dim = rgb_states.size()
        ...
        # state_embed = self.tok_emb(states)
        rgb_embed = self.rgb_emb(rgb_states)
        pc_embed = self.pc_emb(pc_states)

        if self.use_pos_emb:
            rgb_embed += self.pos_emb[:, self.goal_seq_len:(self.goal_seq_len + t), :]
            pc_embed += self.pos_emb[:, self.goal_seq_len:(self.goal_seq_len + t), :]

        rgb_x = self.drop(rgb_embed)
        pc_x = self.drop(pc_embed)
        ...
        if self.goal_conditioned:
            input_seq = torch.cat([emb_t, goal_x, rgb_x, pc_x, action_x], dim=1)
        else:
            input_seq = torch.cat([emb_t, rgb_x, pc_x, action_x], dim=1)
